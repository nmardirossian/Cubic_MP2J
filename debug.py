...
def form_g(mp,mo,row,row_batch):
    ...
    #(_,coords,_,_)=form_coulG(cell,size='full')
    nocc=mp.nocc
    nvirt=mp.nmo-nocc
...
def nm_fft(inp,ngs,griddim):
    return numpy.fft.fftn(inp.reshape(griddim)).reshape(ngs)

def nm_ifft(inp,ngs,griddim):
    return numpy.fft.ifftn(inp.reshape(griddim)).reshape(ngs)

def nm_fft_real(inp,ngs,griddim):
    return numpy.fft.rfftn(inp.reshape(griddim)).reshape(ngs)

def nm_ifft_real(inp,ngs,griddim,griddim2):
    return numpy.fft.irfftn(inp.reshape(griddim),griddim2).reshape(ngs)
...
def get_coulG_new(cell,mesh,smallmesh,size='real'):
    ...
    if size=='full':
        ...
        gz = numpy.fft.rfftfreq(mesh[2], 1./mesh[2])
    #gx = gy = gz = numpy.array([INSERTHERE])
    gxyz = pylib.cartesian_prod((gx, gy, gz))
    b = cell.reciprocal_vectors()
    Gv = numpy.dot(gxyz, b)
    absG2=numpy.sum(Gv*Gv,axis=1)
    with numpy.errstate(divide='ignore'):
        coulG2 = 4.*numpy.pi/absG2
    coulG2[absG2==0] = 0.

    a_frac = numpy.einsum('i,ij->ij', 1./mesh, cell.lattice_vectors())
    coords = numpy.dot(gxyz, a_frac)

    return coulG2,coords

def form_coulG(cell,size='real'):
...
def kernel(mp,mo_energy=None,mo_coeff=None,verbose=logger.NOTE):
    ...
    ngs=mp.ngs
    cell=mp._scf.cell
    (coulG,coords,mesh,smallmesh)=form_coulG(cell,size='real')
    (coulGfull,coordsfull,_,_)=form_coulG(cell,size='full')
    ...
    moERIquick=pbcdf.fft_ao2mo.general(with_df,mo_coeff,compact=False).reshape((nao,)*4)
    correct_energy=0.0
    for tau in range(1):
        for i in range(nocc):
            for a in range(nao-nocc):
                for j in range(nocc):
                    for b in range(nao-nocc):
                        correct_energy-=weightarray[tau]*(2.*moERIquick[i,nocc+a,j,nocc+b]*moERIquick[i,nocc+a,j,nocc+b])*numpy.exp(-(mo_energy[0,:nocc][i]+mo_energy[0,:nocc][j]-mo_energy[0,nocc:][a]-mo_energy[0,nocc:][b])*tauarray[tau])

    numpy.set_printoptions(threshold=numpy.inf,linewidth=15,precision=16)

    coords=cell.gen_uniform_grids(mesh=cell.mesh)
    aoR=numint.eval_ao(cell, coords)
    moR=numpy.asarray(pylib.dot(mo_coeff.T, aoR.T), order='C')
    moRocc=moR[:nocc]
    moRvirt=moR[nocc:]
    ngssmall=numpy.product(smallmesh)

    no_real=0.0
    for i in range(1):
        moRoccW=moRocc*numpy.exp(-mo_energy[:,:nocc].T*tauarray[i]/2.)
        moRvirtW=moRvirt*numpy.exp(mo_energy[:,nocc:].T*tauarray[i]/2.)
        gfunc=pylib.dot(moRoccW.T.conj(),moRoccW)
        gbarfunc=pylib.dot(moRvirtW.T.conj(),moRvirtW)
        ffunc=gfunc*gbarfunc.T
        f2func=numpy.zeros((ngs,ngs),dtype='complex128')
        print 'complex code: ', numpy.linalg.norm(ffunc)
        for j in range(ngs):
            f2func[j]=nm_ifft(nm_fft(ffunc[:,j],ngs,cell.mesh)*coulGfull,ngs,cell.mesh)
            #correctcoulG=(nm_fft_real(f2func[j].real,ngssmall,cell.mesh)/nm_fft_real(ffunc[:,j],ngssmall,cell.mesh)).real
        print 'complex code: ', (numpy.linalg.norm(f2func),numpy.linalg.norm(f2func.real))
        #f2func=f2func*f2func.conj().T
        f2func=f2func.real*f2func.real.T
        Jint=numpy.sum(f2func)
        no_real-=2.*weightarray[i]*Jint*(cell.vol/ngs)**2.
    ...
    for i in range(1):
        ...
        ffunc=gfunc*gbarfunc.T
        f2func=numpy.zeros((ngs,ngs),dtype='complex128')
        print 'real code: ', numpy.linalg.norm(ffunc)
        for j in range(ngs):
            f2func[j]=nm_ifft_real(nm_fft_real(ffunc[:,j],ngssmall,cell.mesh)*coulG,ngs,smallmesh,cell.mesh)
        print 'real code: ', (numpy.linalg.norm(f2func),numpy.linalg.norm(f2func.real))
        f2func=f2func.real*f2func.real.T
        Jint=numpy.sum(f2func)
        with_real-=2.*weightarray[i]*Jint*(cell.vol/ngs)**2.
    ...
    for i in range(numpy.amin([NLapPoints,mp.lt_points])):
        ...
        for c1 in range(grid_batch_num_disk):
            ...
            for gb in range(grid_batch_num):
                ...
                g_o=None
                print "my code: ", numpy.linalg.norm(F)
    ...
    print "Correct energy: ", correct_energy
    print "no_real energy: ", no_real
    print "with_real energy: ", with_real
...
#cell=get_cell(5.5,'C','sc','gth-szv',mesh_val,'gth-pade',supercell=[1,1,1])
scf=get_scf(cell)

#numpy.random.seed(1)
#scf.mo_energy=numpy.random.rand(*numpy.shape(scf.mo_energy))
#scf.mo_coeff=numpy.random.rand(*numpy.shape(scf.mo_coeff))
